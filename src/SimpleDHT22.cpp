...
void SimpleDHT22::resetTimingDefaults() {
    _startSignal = 1100;      // 1.1ms start signal (per datasheet: 1-10ms)
    _responseTimeout = 200;   // 200us timeout for sensor response (80us low + 80us high)
    _bitTimeout = 100;        // 100us timeout for bit signals
    _bitThreshold = 50;       // 50us threshold for bit decision (>50us = 1, <50us = 0)
}
